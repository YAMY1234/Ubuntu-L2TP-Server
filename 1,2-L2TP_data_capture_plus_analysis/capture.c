...
)
{
    ...
    l2tp_type = ((*(l2tp_header)) & 0x80) >> 7;//01000000
    l2tp_len_field = ((*(l2tp_header)) & 0x40) >> 6;//00100000
    l2tp_sequence_field = ((*(l2tp_header)) & 0x08) >> 3;//00000100
    l2tp_offset_field = ((*(l2tp_header)) & 0x02) >> 1;//00000010
    ...
    if (l2tp_version == 0x02)
        ...
        printf("L2TP Version is%d", l2tp_version);
        return;
    }
    if (l2tp_type)
...
int ip_subnet_mask_print(bpf_u_int32 ip, bpf_u_int32 subnet_mask) {
    ...
    */
    ...
    address.s_addr = ip;//感觉这个语句并没有什么用
...
int main(int argc, char **argv) {
    ...
    // create handle，执行到这个地方，开始正式的展开pcap的捕获
    ...
    //最为核心的代码捕获的函数
    pcap_loop(handle, total_packet_count, my_packet_handler, my_arguments);//这些信息应该都从这里面来的，所以说默认了这些都是有的
